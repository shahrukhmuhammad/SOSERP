...
$('#carousel').flexslider({
  ...
  $('.product_images').flexslider({
    ...
    selectCallback = function (variant, selector) {
      if(variant) {
        // set variant title in product title if not default title, handle multiple variants
        if (variant.option1 && variant.option1 !== 'Default Title') {
          var variantTitle = ' - ' + variant.option1;
        }
        if(variant.option2) {
        ...
        $('#productTitleVariant').text(variantTitle);
    ...
    if (variant) {
      if (variant.inventory_management == "shopify" && variant.inventory_policy != "continue") {
        if (variant.inventory_quantity >0) {
          jQuery('#variant-inventory').html('<span class=in-stock>In stock! Order by 2:00pm for next day delivery.</span>');
        } else {
