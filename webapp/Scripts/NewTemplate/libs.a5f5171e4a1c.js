...
;( function( window ) {
	...
	mlPushMenu.prototype = {
		...
		_resetMenu : function() {
			...
			this._toggleLevels();
			this.open = false;
                        classie.remove( this.el, 'mp-opened' );
		},
		// close sub menus
		_closeMenu : function() {
...
(function() {
  ...
  Chosen = (function(_super) {
    ...
    Chosen.prototype.register_observers = function() {
      ...
      this.container.bind('mousedown touchstart', function(evt) {
        _this.container_mousedown(evt);
      });
      this.container.bind('mouseup.chosen touchend', function(evt) {
        _this.container_mouseup(evt);
      });
      this.container.bind('mouseenter.chosen', function(evt) {
        _this.mouse_enter(evt);
      });
      this.container.bind('mouseleave.chosen touchend', function(evt) {
        _this.mouse_leave(evt);
      });
      this.search_results.bind('mouseup.chosen touchend', function(evt) {
      ...
      if (!this.is_disabled) {
        ...
        if (!((evt != null) && ($(evt.target)).hasClass("search-choice-close"))) {
          if (!this.active_field) {
            ...
            $(document).bind('click.chosen touchstart', this.click_test_action);
            this.results_show();
    ...
    Chosen.prototype.close_field = function() {
      $(document).unbind("click.chosen touchstart", this.click_test_action);
      this.active_field = false;
      this.results_hide();
    ...
    Chosen.prototype.set_label_behavior = function() {
      ...
      if (this.form_field_label.length > 0) {
        return this.form_field_label.bind('click.chosen touchstart', function(evt) {
...
//			console.log(elem.parent());
			...
			function initialiseVerticalScroll()
			{
				if (isScrollableV) {
					...
					if(elem.parent().hasClass('-list-container'))
					{

							verticalTrackHeight = paneHeight-60;
					}
...
(function() {
  var CookieCache, jStorageCache;

  jStorageCache = (function() {

    function jStorageCache() {}

    jStorageCache.is_available = function() {
      return $.jStorage.storageAvailable();
    };

    jStorageCache.prototype.get = function(key, def) {
      if (def == null) {
        def = null;
      }
    ...
    jStorageCache.prototype.set = function(key, value, timeout) {
      var ttl;
      if (timeout == null) {
        timeout = null;
      }
    ...
    jStorageCache.prototype["delete"] = function(key) {
      return $.jStorage.deleteKey(key);
    };

    jStorageCache.prototype.clear = function() {
      return $.jStorage.flush();
    };

    return jStorageCache;

  })();

  CookieCache = (function() {

    function CookieCache() {}

    CookieCache.KEY_PREFIX = 'cachecookie_';

    CookieCache.prototype.get = function(key, def) {
      var val;
      if (def == null) {
        def = null;
      }
      val = $.cookie(CookieCache.KEY_PREFIX + key);
      if (val != null) {
        return val;
      } else {
        return def;
    ...
    CookieCache.prototype.set = function(key, value, timeout) {
      var ttl;
      if (timeout == null) {
        timeout = null;
      }
      if (timeout != null) {
        ttl = timeout * 1000;
        return $.cookie(CookieCache.KEY_PREFIX + key, value, {
          path: '/',
    ...
    CookieCache.prototype["delete"] = function(key) {
      return $.removeCookie(CookieCache.KEY_PREFIX + key);
    };

    CookieCache.prototype.clear = function() {
      var _this = this;
      return $.each($.cookie(), function(key, value) {
        if (key.indexOf(CookieCache.KEY_PREFIX, 0)) {
          return _this["delete"](key.substring(CookieCache.KEY_PREFIX.length));
        }
      });
    };
  ...
  window.cache = jStorageCache.is_available() ? new jStorageCache() : new CookieCache();

}).call(this);

// Generated by CoffeeScript 1.6.1
(function() {
  var Geolocation, LIST_URL_PREFIX, ObjectLocation, UserLocation;

  LIST_URL_PREFIX = '/lekarze/';

  UserLocation = (function() {

    function UserLocation(lat, lon, city, city_slug, district, district_slug, accuracy) {
      this.lat = lat;
      this.lon = lon;
      this.city = city;
      this.city_slug = city_slug;
      this.district = district;
      this.district_slug = district_slug;
  ...
  Geolocation = (function() {
    ...
    Geolocation.CACHE_KEY_HASLOCATION_FLAG = 'haslocation';

    Geolocation.CACHE_KEY_LOCATION = 'userlocation';

    Geolocation.LOCATION_TTL = 1800;

    Geolocation.LOCATION_CACHE_TTL = 300;

    Geolocation.SERVICE_URL = '/geolocation/';

    Geolocation.GEOIP_SERVICES_WIKIMEDIA = 'http://geoiplookup.wikimedia.org/';

    Geolocation.GEOIP_SERVICES_FREEGEOIP = 'http://freegeoip.net/json/';

    Geolocation.HTML5_WAIT_TIMEOUT = 10 * 1000;
    ...
    Geolocation.prototype.getUserDistance = function(lat, lon, callback) {
      var _this = this;
      return this.getUserLocation(false, function(loc) {
        return callback(_this.getPointsDistance(parseFloat(lat), parseFloat(lon), loc.lat, loc.lon));
      });
    };

    Geolocation.prototype.getUserLocation2 = function(callback) {
      var haslocation, html5locErr, html5locSucc,
        _this = this;
      haslocation = cache.get(Geolocation.CACHE_KEY_HASLOCATION_FLAG);
      html5locSucc = function(position) {
        return _this.setLocation(position.coords.latitude, position.coords.longitude, position.coords.accuracy, callback);
      };
      html5locErr = function() {
        if (haslocation) {
          return _this.getLocation(callback);
        } else {
          return _this.getGeoIpLocation(callback, function() {
            return this.setLocation(Geolocation.DEFAULT_LOC.lat, Geolocation.DEFAULT_LOC.lon, DEFAULT_LOC.accu, callback);
          });
        }
      };
      if (navigator && navigator.geolocation) {
        return navigator.geolocation.getCurrentPosition(html5locSucc, html5locErr);
      }
    };

    Geolocation.prototype.getUserLocation = function(refreshFromHTML, callback) {
      var haslocation,
        _this = this;
      if (refreshFromHTML == null) {
        refreshFromHTML = false;
      }
      haslocation = cache.get(Geolocation.CACHE_KEY_HASLOCATION_FLAG);
      if (!refreshFromHTML && (haslocation != null)) {
        if (callback != null) {
      ...
      if (refreshFromHTML) {
        return this.getHTML5Location(callback, function() {
          return _this.getGeoIpLocation(callback, (function() {
            return _this.getLocation(callback);
          }));
        });
      } else {
        return this.getGeoIpLocation(callback, (function() {
          return _this.getLocation(callback);
        }));
      }
    };

    Geolocation.prototype.getGeoIpLocation = function(onSuccess, onError) {
      var _this = this;
      return this.getGeoIpLocationFreegeoip(onSuccess, function() {
        return _this.getGeoIpLocationWikimedia(onSuccess, onError);
      });
    };

    Geolocation.prototype.getGeoIpLocationWikimedia = function(onSuccess, onError) {
      var encodedQuery, query, success, url,
        _this = this;
      success = function(data) {
        var resp;
        if ((data.query == null) || (data.query.results == null) || (data.query.results.body == null) || !data.query.results.body.country !== 'PL') {
          return onError();
        }
        resp = $.parseJSON(data.query.results.body.split('=')[1]);
        return _this.setLocation(parseFloat(resp.lat), parseFloat(resp.lon), null, onSuccess, onError);
      };
      query = "select * from html where url='" + Geolocation.GEOIP_SERVICES_WIKIMEDIA + "'";
      ...
      return $.get(url, success, 'json').fail(onError);
    };

    Geolocation.prototype.getGeoIpLocationFreegeoip = function(onSuccess, onError) {
      var success,
        _this = this;
      success = function(data) {
        ...
        return _this.setLocation(parseFloat(data.latitude), parseFloat(data.longitude), null, onSuccess, onError);
      };
      return $.get(Geolocation.GEOIP_SERVICES_FREEGEOIP, success, 'json').fail(onError);
    };

    Geolocation.prototype.getHTML5Location = function(onSuccess, onError) {
      ...
      locationSuccess = function(position) {
        return _this.setLocation(position.coords.latitude, position.coords.longitude, position.coords.accuracy, onSuccess, onError);
      };
      locationError = function(err) {
        return onError();
      };
      return navigator.geolocation.getCurrentPosition(locationSuccess, locationError, {
        timeout: Geolocation.HTML5_WAIT_TIMEOUT,
        enableHighAccuracy: true
      });
    };

    Geolocation.prototype.getLocation = function(onSuccess, onError) {
      var cached, success;
      success = function(data, setCache) {
        if (setCache == null) {
          setCache = true;
        }
        if (setCache) {
          cache.set(Geolocation.CACHE_KEY_LOCATION, data, Geolocation.LOCATION_CACHE_TTL);
        }
        return onSuccess(new UserLocation(data.lat, data.lon, data.city, data.city_slug, data.district, data.district_slug, data.accuracy));
      };
      cached = cache.get(Geolocation.CACHE_KEY_LOCATION);
      if (cached != null) {
        success(cached, false);
        return;
      }
      return $.getJSON(Geolocation.SERVICE_URL, success).fail(onError);
    };

    Geolocation.prototype.setLocation = function(lat, lon, accuracy, onSuccess, onError) {
      var params, success;
      success = function(data) {
        cache.set(Geolocation.CACHE_KEY_HASLOCATION_FLAG, true, Geolocation.LOCATION_TTL);
        cache.set(Geolocation.CACHE_KEY_LOCATION, data, Geolocation.LOCATION_CACHE_TTL);
        return onSuccess(new UserLocation(data.lat, data.lon, data.city, data.city_slug, data.district, data.district_slug, data.accuracy));
      };
      ...
      return $.post("" + Geolocation.SERVICE_URL + "?" + params, success).fail(onError);
    };

    Geolocation.prototype.getPointsDistance = function(lat1, lon1, lat2, lon2) {
      var CI, R, a, c, dLat, dLon, sinDLat, sinDLon;
      R = 6371;
      CI = 0.017453292519943295;
      dLat = (lat2 - lat1) * CI / 2;
      dLon = (lon2 - lon1) * CI / 2;
  ...
  ObjectLocation = (function() {
    ...
    ObjectLocation.prototype.KIND_AGENCY = 'agency';

    ObjectLocation.prototype.SERVICE_URL = "" + Geolocation.SERVICE_URL + "obj/";

    function ObjectLocation() {
      this.geocoder = typeof google !== "undefined" && google !== null ? new google.maps.Geocoder() : null;
    }

    ObjectLocation.prototype.getAddressLocation = function(address, callback, objectId, objectKind, notFoundCallback) {
      var addresses, cb, findLocation,
        _this = this;
      if (objectId == null) {
        objectId = null;
      }
      ...
      if (_.isNumber(objectId)) {
        cb = function(lat, lon) {
          _this.saveLocation(objectId, objectKind, lat, lon);
      ...
      findLocation = function(i) {
        ...
        return _this.geocoder.geocode({
          ...
          if (status === google.maps.GeocoderStatus.OK) {
            return cb(results[0].geometry.location.lat(), results[0].geometry.location.lng());
          } else {
            if ((addresses.length - 1) > i) {
              return findLocation(i + 1);
            } else {
              if (notFoundCallback != null) {
    ...
    ObjectLocation.prototype.saveLocation = function(objectId, objectKind, lat, lon) {
      return $.ajax({
        type: 'POST',
        url: "" + this.SERVICE_URL + objectKind + "/",
        data: {
          r: $.base64.encode("" + lat + ";" + lon + ";" + objectId)
        }
      });
    };
  ...
  window.objectlocation = new ObjectLocation();

  window.calculateObjectsDistance = function() {
    return $('.calculate-distance:not(.calculated)').each(function() {
      var el;
      el = $(this);
      return objectlocation.getAddressLocation($(this).data('address'), function(lat, lon) {
        return geolocation.getUserDistance(lat, lon, function(distance) {
          el.addClass('calculated');
          el.find('.distance-km').text(Math.round(distance * 100) / 100);
          return el.show();
        });
      });
    });
  ...
  window.handleHomePage = function() {
    var $chLink, $inp, $link, setHomePageLoc;
    $link = $('#rl-geoloc-link');
    $inp = $('#rl-geoloc-input');
    $chLink = $('#rl-geoloc-change');
    setHomePageLoc = function(text, lat, lon, slug) {
      if (slug != null) {
        $link.attr('href', LIST_URL_PREFIX + slug);
      } else {
        $link.attr('href', LIST_URL_PREFIX + text);
      }
      $inp.data('lon', lon);
      $inp.data('lat', lat);
      return $inp.val(text[0].toUpperCase() + text.substr(1));
    };
    if ($link.length > 0) {
      geolocation.getUserLocation2(function(data) {
        return setHomePageLoc(data.city, data.lat, data.lon);
      });
      $inp.yourlabsAutocomplete({
        url: $inp.data('url'),
        choiceSelector: 'span.div',
        placeholder: '',
        minimumCharacters: 2
      });
      $inp.bind('selectChoice', function(e, choice, autocompl) {
        return setHomePageLoc(choice.text().split(',')[0], choice.data('lat'), choice.data('lon'), choice.data('slug'));
      });
      return $chLink.click(function() {
        return geolocation.setLocation($inp.data('lat'), $inp.data('lon'), null, function(data) {
          setHomePageLoc(data.city, data.lat, data.lon);
          return window.location = $inp.data('search-url') + data.city;
        });
      });
    }
  };
...
(function() {
  ...
  SessionSharing = (function() {
    ...
    function SessionSharing() {}

    SessionSharing.API_URL = '/session/ss/';

    SessionSharing.CHECK_EVERY = 120;

    SessionSharing.CACHE_KEY = 'sschecked';

    SessionSharing.JUST_AUTH_COOKIE_NAME = 'just_authenticated';

    SessionSharing.prototype.perform = function() {
      var cache_key, currentHost, just_authenticated;
      just_authenticated = $.removeCookie(SessionSharing.JUST_AUTH_COOKIE_NAME);
      currentHost = this.getHost();
      cache_key = "" + SessionSharing.CACHE_KEY + currentHost;
      if (!just_authenticated && cache.get(cache_key)) {
        return;
      }
      //return $.getJSON(SessionSharing.API_URL, function(data) {
      ...
      //      _results.push($.ajax({
      //        type: "POST",
      //        url: "" + host + SessionSharing.API_URL,
      //        data: {
      //          t: data.token
      //        },
      //        xhrFields: {
      ...
      //        },
      //        success: function() {
      //          return cache.set(cache_key, true, SessionSharing.CHECK_EVERY);
      //        }
      //      }));
      //    } else {
    ...
    SessionSharing.prototype.getHost = function() {
      var currentHost;
  ...
  window.sessionsharing = new SessionSharing();

}).call(this);

// Generated by CoffeeScript 1.6.1
(function() {
  var allow_geo, bounds, directionsDisplay, directionsService, findLocation, geocoder, i, locationError, locationSuccess, refreshFromHTML, retries, setAllLocations, userLocation, userLocationFromHTML, userLocationMarker, waitForLocation;
  ...
  window.showLoader = function() {
    return $('.ui-loader').show();
  };

  window.hideLoader = function() {
    return $('.ui-loader').hide();
  };

  $(function() {
    return sessionsharing.perform();
  });

  $("#fixed-messages .close").on("click touchstart", function(e) {
    e.preventDefault();
    return $("body").removeClass("show-fixed-message");
  });
  ...
  findLocation = function() {
    return geocoder.geocode({
      address: but.data('address')
    }, function(results, status) {
      if (status === google.maps.GeocoderStatus.OK) {
        setAgency(results[0].geometry.location.lat(), results[0].geometry.location.lng());
        but.data('lat', results[0].geometry.location.lat());
        return but.data('lon', results[0].geometry.location.lng());
      }
    });
  };
  ...
  locationSuccess = function(loc) {
    ...
    $.cookie('allow_geo_localization', 'true', {
      expires: 7,
      path: '/'
    });
    allowFromHTML = true;
    window.allowFromHTML = true;
    return setAllLocations();
  };

  window.allowFromHTML = false;

  locationError = function(err) {
    var allow_geo, not_allowed_geo_message;
    $.removeCookie('allow_geo_localization');
    $.cookie('allow_geo_localization', 'false', {
      expires: 7,
      path: '/'
    });
    if (err.code === 1) {
      allow_geo = $.cookie('allow_geo_localization');
      if (allow_geo !== "true") {
        not_allowed_geo_message = "<div class='message warning'><h5>Proszę zmienić ustawienia geolokalizacji w przeglądarce.</h5></div>";
        setTimeout((function() {
          $("#fixed-messages .message").remove();
          $("#fixed-messages").prepend(not_allowed_geo_message);
          $("body").addClass("show-fixed-message");
        }), 200);
        $.removeCookie('allow_geo_localization', {
          path: '/'
        });
    ...
    if (err.code !== 1) {
      getUserLocation(false, false, null);
      waitForLocation();
      $.removeCookie('allow_geo_localization', {
        path: '/'
      });
      return $.cookie('allow_geo_localization', 'true', {
        expires: 7,
        path: '/'
      });
    }
  ...
  window.getUserLocation = function(refreshFromHTML, setMarker, cb) {
    if (typeof google !== "undefined") {
      return geolocation.getUserLocation(refreshFromHTML, function(loc) {
        userLocation = new google.maps.LatLng(loc.lat, loc.lon);
        userLocationFromHTML = refreshFromHTML;
        if (setMarker) {
  ...
  $("span.gray").on("click touchstart", function(e) {
    if (navigator.geolocation) {
      return window.watching = navigator.geolocation.getCurrentPosition(locationSuccess, locationError, {
        timeout: 10 * 1000,
        enableHighAccuracy: true
      });
    }
  ...
  setAllLocations = function() {
    return $("span.gray").each(function() {
      var but, lat, latlon, lon, setDirections;
      i++;
      latlon = $(this).data('latlon').split(',');
      ...
      setDirections = function() {
        ...
        return directionsService.route(request, function(response, status) {
          var distance_km;
          if (response.routes && response.routes[0].legs && response.routes[0].legs[0].distance) {
            distance_km = response.routes[0].legs[0].distance.value / 1000;
            but.text("(" + (Math.round(distance_km * 100) / 100) + " km)").addClass("done");
          }
          return hideLoader();
        });
      };
  ...
  if (navigator.geolocation && allow_geo === 'true') {
    window.watching = navigator.geolocation.getCurrentPosition(locationSuccess, locationError, {
      timeout: 10 * 1000,
      enableHighAccuracy: true,
      maximumAge: 5000
    });
  ...
  waitForLocation = function() {
    if (userLocation === null && retries < 10) {
  ...
  window.lower_star = function() {
    $(".doctor-data").each(function() {
      var h1_width, rating_width, t, width;
      t = $(this);
      width = t.width();
      h1_width = t.find("h2").width();
      rating_width = t.find(".rating").width();
      if (rating_width + h1_width > width) {
        t.find(".rating").addClass("lower");
      } else {
        t.find(".rating").removeClass("lower");
      }
    });
  };
