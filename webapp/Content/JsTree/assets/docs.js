...
(function ($) {
	...
	$(function () {
		...
		$.address
			...
			.init(function(e) {
			    $('#docs a:not([href^=http])').not($('.demo a')).address().on('click', function () { if ($.address.pathNames().length < 2 && !$.address.parameter('f')) { $(document).scrollTop(0); } });
			})
			.change(function(e) {
				...
				$('#menu').find('#docs a[href$="' + page + '"]').blur().parent().addClass('active').siblings().removeClass('active');
		...
		$.getJSON('/Content/JsTree/jstree.json', function (data) {
			//return;
